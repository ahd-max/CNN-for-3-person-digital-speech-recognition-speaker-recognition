# -*- coding: utf-8 -*-
"""
Created on Sat Oct 29 17:16:32 2022
...
class Network():
    ...
    def add_conv(self, *, patch_size, in_depth, out_depth, activation='relu', pooling=False, name):
        ...
        with tf.name_scope(name):
            weights = tf.Variable(
                tf.random.truncated_normal([patch_size, patch_size, in_depth, out_depth], stddev=0.01), name=name + '_weights')
            biases = tf.Variable(tf.constant(0.01, shape=[out_depth]), name=name + '_biases')
    ...
    def add_fc(self, *, in_num_nodes, out_num_nodes, activation='relu', name):
        ...
        with tf.name_scope(name):
            weights = tf.Variable(tf.random.truncated_normal([in_num_nodes, out_num_nodes], stddev=0.1))
    ...
    def define_inputs(self, *, train_samples_shape, train_labels_shape, test_samples_shape):
        ...
        with tf.name_scope('inputs'):
            # self.tf_train_samples = tf.compat.v1.disable_eager_execution(tf.float32, shape=train_samples_shape, name='tf_train_samples')
            # self.tf_train_labels = tf.compat.v1.disable_eager_execution(tf.float32, shape=train_labels_shape, name='tf_train_labels')
            # self.tf_test_samples = tf.compat.v1.disable_eager_execution(tf.float32, shape=test_samples_shape, name='tf_test_samples')
            
            self.tf_train_samples = tf.compat.v1.placeholder(tf.float32, shape=train_samples_shape, name='tf_train_samples')
            self.tf_train_labels = tf.compat.v1.placeholder(tf.float32, shape=train_labels_shape, name='tf_train_labels')
            self.tf_test_samples = tf.compat.v1.placeholder(tf.float32, shape=test_samples_shape, name='tf_test_samples')
    ...
    def define_model(self):
            ...
            for i, (weights, biases, config) in enumerate(zip(self.conv_weights, self.conv_biases, self.conv_config)):
                with tf.name_scope(config['name'] + '_model'):
                    with tf.name_scope('convolution'):
                        ...
                        data_flow = tf.nn.conv2d(data_flow, weights, strides=[1, 1, 1, 1], padding='SAME')
        ...
        with tf.name_scope('optimizer'):
            if (self.optimizeMethod == 'gradient'):
                self.optimizer = tf.compat.v1.train \
        ...
        with tf.name_scope('train'):
            ...
            tf.compat.v1.add_to_collection("prediction", self.train_prediction)
        with tf.name_scope('test'):
            self.test_prediction = tf.nn.softmax(model(self.tf_test_samples, train=False), name='test_prediction')
            tf.compat.v1.add_to_collection("prediction", self.test_prediction)

            single_shape = (1, 32, 32, 1)   ##图片格式
            #single_shape = (1, 64, 32, 1)  ##图片格式
            #single_shape = (1, 64, 64, 1)   ##图片格式
            single_input = tf.compat.v1.placeholder(tf.float32, shape=single_shape, name='single_input')
            self.single_prediction = tf.nn.softmax(model(single_input, train=False), name='single_prediction')
            tf.compat.v1.add_to_collection("prediction", self.single_prediction)
    ...
    def visualize_filter_map(self, tensor, *, how_many, display_size, name):
        ...
        # print(how_many)
        self.test_summaries.append(tf.summary.image(name, filter_map, max_outputs=how_many))
